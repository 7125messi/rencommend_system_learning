        ...
        for i in range(self.input_dim):
            for j in range(i+1,self.input_dim):
                weight = tf.math.reduce_sum(
                    tf.math.multiply(
                        self.kernel[i, self.field_dict[j]], 
                        self.kernel[j, self.field_dict[i]]
                    )
                )
...
def train():
    ...
    X_train, X_test, y_train, y_test = train_test_split(data.data, data.target, test_size=0.2,random_state=27,stratify=data.target)
